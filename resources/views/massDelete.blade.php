...
@section('content')
    ...
    <div class="d-flex justify-content-center align-items-center">
        <div class="w-75 p-3 translucent-bg custom-container mb-4">
            <form action="{{ route('massDelete.push') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="d-flex flex-column align-items-center mb-4">
                    <label for="formFileLg" class="pointer mb-2">Вставьте Excel Файл с ean которые вы хотите удалить</label>
                    <input class="w-50 pointer" id="formFileLg" type="file" name="excel" />
                </div>
                <div class="h-100 d-flex flex-column justify-content-center align-items-center">
                    <p class="">Из каких баз JV нужно удалить?</p>
                    ...
                    <div class="w-100 d-flex justify-content-between">
                        @foreach ($jv as $key => $siteName)
                            <div class="checkboxes__item">
                                <label class="checkbox style-d">
                                    <input type="checkbox" value="{{ $key }}" name="bases[]"
                                        id="jvCheck{{ $count }}" />
                                    <div class="checkbox__checkmark"></div>
                                    <div class="checkbox__body">{{ $siteName }}</div>
                                </label>
                            </div>
                            @php
                                $count++;
